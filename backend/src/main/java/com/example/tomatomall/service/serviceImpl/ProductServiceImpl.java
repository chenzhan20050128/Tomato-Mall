...
异常捕获：捕获分布式锁和缓存操作异常，降级到数据库查询，确保最终可用性。
    ...
    public List<Product> getAllProducts() {
        ...
        ValueOperations<String, List<Product>> ops = productListRedisTemplate.opsForValue();
        List<Product> cachedProducts = ops.get(PRODUCT_LIST_CACHE_KEY);

        if (cachedProducts != null && !cachedProducts.isEmpty()) {
            if (log.isDebugEnabled()) {
                log.debug("从Redis缓存获取商品列表");
            }
        ...
        RLock lock = redissonClient.getLock("product_list_lock");
        try {
            // 尝试非阻塞获取锁，设置合理的等待时间
            if (lock.tryLock(100, 3000, TimeUnit.MILLISECONDS)) {
                try {
                    // 双重检查
                    cachedProducts = ops.get(PRODUCT_LIST_CACHE_KEY);
                    if (cachedProducts != null && !cachedProducts.isEmpty()) {
                        return cachedProducts;
                    }
                    ...
                    do {
                        ...
                        productPage = productRepository.findAllWithSpecifications(pageable);
                    ...
                    if (products.isEmpty()) {
                        // 缓存空值防止穿透
                        ops.set(PRODUCT_LIST_CACHE_KEY, Collections.emptyList(),
                                NULL_CACHE_TTL.toMillis(), TimeUnit.MILLISECONDS);
                        return Collections.emptyList();
                    }
                    ...
                    CompletableFuture.runAsync(() -> {
                        try {
                            ...
                            Map<String, List<Product>> cacheMap = new HashMap<>();

                            for (int i = 0; i < chunks.size(); i++) {
                                String chunkKey = PRODUCT_LIST_CACHE_KEY + "::chunk_" + i;
                                cacheMap.put(chunkKey, chunks.get(i));
                            }

                            // 使用pipeline批量写入
                            productListRedisTemplate.executePipelined((RedisCallback<Object>) connection -> {
                                cacheMap.forEach((k, v) ->
                                        ops.set(k, v,
                                                CACHE_TTL.toMillis() + ThreadLocalRandom.current().nextInt(30_000),
                                                TimeUnit.MILLISECONDS));
                                return null;
                            });

                            // 设置分片元数据
                            String metaKey = PRODUCT_LIST_CACHE_KEY + "::meta";
                            Map<String, Object> meta = new HashMap<>();
                            meta.put("chunk_count", chunks.size());
                            meta.put("expire_at", System.currentTimeMillis() +
                                    CACHE_TTL.toMillis() + ThreadLocalRandom.current().nextInt(30_000));
                            productListRedisTemplate.opsForHash().putAll(metaKey, meta);
                            productListRedisTemplate.expire(metaKey, CACHE_TTL.plusSeconds(30).getSeconds(), TimeUnit.SECONDS);
                        } catch (Exception e) {
            ...
            log.error("缓存处理异常，降级到数据库查询", e);
            // 降级策略：直接查询数据库
            return productRepository.findAllWithSpecifications(PageRequest.of(0, 100)).getContent();
        }
