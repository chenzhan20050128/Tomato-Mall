...
public class OrderController {
    ...
    // 修改支付接口为POST方法
    @PostMapping("/{orderId}/pay") // 原为@GetMapping("/pay")
    public void pay(@PathVariable Integer orderId, HttpServletResponse httpResponse) throws Exception {
        // 需要从数据库获取订单信息替换aliPay参数
        Order order = orderService.getOrderById(orderId);
        AliPay aliPay = new AliPay();
        aliPay.setTraceNo(order.getOrderId().toString());
        aliPay.setTotalAmount(order.getTotalAmount().doubleValue());
        aliPay.setSubject("Tomato Mall Order #" + orderId);
